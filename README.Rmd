...
This is a personal package for themes and theme components for ggplot2::ggplot2, gt, and other tools as I discover and use them throughout R.
...
The following functions for ggplot2::ggplot2 are implemented/objects are exported:
...
- `DGThemes::scale_color_premium`: Discrete color & fill scales based on the premium palette
- `scale_colour_premium`:  Discrete color & fill scales based on the premium palette
- `DGThemes::scale_fill_premium`:  Discrete color & fill scales based on the premium palette
...
```{r at}
ggplot2::ggplot(tophitters2001, aes(hr, so)) +
  ggplot2::geom_point() +
  ggplot2::labs(
  ...
  ggplot2::geom_text(aes(label = dplyr::if_else(hr > 50, name, NULL)),
    vjust = 1.2
  ) +
...
```{r rc}
ggplot2::ggplot(beaver1, aes(time, temp)) +
  ggplot2::geom_line() +
  ggplot2::labs(
...
ggplot2::ggplot(Seatbelts, aes(DriversKilled, kms)) +
  ggplot2::geom_point(aes(color = factor(law))) +
  ggplot2::labs(
...
ggplot2::ggplot(diamonds, aes(carat, price)) +
  ggplot2::geom_jitter(aes(color = clarity, fill = clarity), size = 3, shape = 21, alpha = 1 / 2) +
...
```{r sc}
ggplot2::ggplot(tophitters2001, aes(hr, so, color = lg)) +
  ggplot2::geom_point() +
  ggplot2::labs(
    x = "Home Runs", y = "Strikeouts",
    title = "Seminal ggplot2::ggplot2 scatterplot example",
  ...
  ggplot2::geom_text(aes(label = if_else(hr > 50, name, NULL)),
    vjust = 1.2
  ) +
  DGThemes::scale_color_premium() +
  theme_premium() +
...
tophitters2001 %>%
  ...
  dplyr::slice(c(1:10)) %>%
  ggplot2::ggplot(aes(fct_reorder(name, pct), pct)) +
  ggplot2::geom_col() +
  ggplot2::geom_text(aes(label = paste0(hr, "\n(", round(pct, 2), "%)")), vjust = -0.8, color = "black") +
  ggplot2::scale_y_percent(scale = 1, limits = c(0, 3)) +
  ggplot2::labs(
...
```{r ax2, fig.height=6}
ggplot2::ggplot(uspopage, aes(x = Year, y = Thousands, fill = AgeGroup)) +
  ggplot2::geom_area() +
  DGThemes::scale_fill_premium() +
  ggplot2::scale_x_continuous(expand = c(0, 0)) +
  ggplot2::scale_y_comma() +
  ggplot2::labs(
...
count(mpg, class) %>%
  ...
  ggplot2::ggplot(aes(class, n)) +
  ggplot2::geom_col() +
  ggplot2::geom_text(aes(label = scales::comma(n)), hjust = 0, nudge_y = 2000) +
  ggplot2::scale_y_comma(limits = c(0, 150000)) +
  ggplot2::coord_flip() +
  ggplot2::labs(
