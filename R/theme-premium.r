...
#'
#' @md
#' @section Why Tisa Sans Pro?:
#' First and foremost, Tisa Sans Pro is generally installed by default or readily
...
#'
#' @section Building upon `theme_premium`:
...
#' }
theme_premium <- function(base_family="Tisa Sans Pro", base_size = 11.5,
                        ...
                        axis_col = "#cccccc", axis = FALSE, ticks = TRUE) {
  ...
  if (!ticks) {
    ...
    ret <- ret + theme(axis.ticks = element_line(size=0.15, color = "black"))
    ret <- ret + theme(axis.ticks.x = element_line(size=0.5, color = "black"))
    ret <- ret + theme(axis.ticks.y = element_line(size=0.5, color = "black"))
...
#' @export
update_geom_font_defaults <- function(family="Tisa Sans Pro", face="plain", size=3.5,
                                      color = "#2b2b2b") {
